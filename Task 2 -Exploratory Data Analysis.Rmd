...
# if (!exists(CACHE_FILE) || is.null(basic_report)) 
{
  ...
  line_counts <- sapply(paste(dir, filelist, sep = "/"), function(x) length(readLines(x, warn = FALSE)))
  
  # Count words per file
  word_counts <- sapply(paste(dir, filelist, sep = "/"), function(x) sum(str_count(readLines(x, warn = FALSE), "\\S+")))
  
  basic_report <- as.data.frame(lapply(basic_report, format_number))
  rownames(basic_report) <- NULL
  names(basic_report) <- filelist
